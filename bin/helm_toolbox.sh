...
function upsert() {
    ...
    if [ "$helm_version" != "$tiller_version" ]; then
        ...
        local tiller_serviceaccount_exists=$(kubectl get serviceaccount -n kube-system --ignore-not-found=false --request-timeout=${TIMEOUT}s $tiller_serviceaccount 2> /dev/null)
        local tiller_clusterrolebinding_exists=$(kubectl get clusterrolebinding --ignore-not-found=false --request-timeout=${TIMEOUT}s tiller-cluster-rule 2> /dev/null)
        if [ -z $tiller_serviceaccount_exists ] && [ -z $tiller_clusterrolebinding_exists ]; then
